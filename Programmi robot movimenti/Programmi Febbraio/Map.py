...
def movimento_30(direzione):
    AS.counter=0
    setta(2200,direzione)
    while(AS.counter<cm):
        time.sleep(0.01)
    setta(0,direzione)
    if(dati.counter==0): dati.riga+=1
    if(dati.counter==1): dati.colonna+=1
    if(dati.counter==2): dati.riga-=1
    if(dati.counter==3): dati.colonna-=1
